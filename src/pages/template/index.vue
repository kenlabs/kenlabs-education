  ...
  <v-flex grow>
    ...
    >
      ...
      <template #rowActions="{ item }">
        <v-btn v-permission:template:update color="primary" small text @click="$refs.table.openUpdateForm(item)">修改
        </v-btn>
      </template>
      <template #batchActions="{ selected }">
        <v-btn v-permission:template:delete :disabled="!selected.length" color="red" text
               @click="$refs.table.openDeleteDialog(selected)"> 删除
        </v-btn>
      </template>
      <template #form="{ item }">
        <v-row dense>
          ...
          <v-col cols="12">
            <v-autocomplete v-model="item.labels" label="模板标签" small-chips :items="options.labels" dense multiple
                            outlined hide-details deletable-chips
